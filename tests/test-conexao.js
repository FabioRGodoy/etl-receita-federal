  ...
  try {
    ...
    if (result.success) {
      ...
      try {
        ...
        hasETLDb = dbResult.rows.some(
          (row) => row.datname === "etl_receita_federal"
        );
      ...
      if (!hasETLDb) {
        ...
        console.log(
          "   1. Criar banco: createdb -h $DB_HOST -p $DB_PORT -U $DB_USER etl_receita_federal"
        );
        console.log(
          '   2. Atualizar DB_NAME no .env para "etl_receita_federal"'
        );
        console.log(
          "   3. Criar schema: psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d etl_receita_federal -f sql/schema.sql"
        );
